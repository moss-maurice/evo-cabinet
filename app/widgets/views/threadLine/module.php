...
<link rel="stylesheet" type="text/css"
    href="<?= App::getPublicWebRoot('/assets/libs/dropzone-5.9.3/min/dropzone.min.css'); ?>?v=<?= time(); ?>">
<link rel="stylesheet" type="text/css"
    href="<?= App::getPublicWebRoot('/widgets/views/threadLine/assets/styles/style.css'); ?>?v=<?= time(); ?>">
    ...
    <div class="col-12 col-lg-6">
        ...
        <div class="files-control d-block">
            <form class="dropzone needsclick" id="media-upload" action="/upload">
                <div class="dz-message needsclick">Перетащите файл в это поле или кликните мышкой для загрузки файла
                </div>
            </form>
        </div>
        <div class="add-control d-block">
            <span class="btn btn-success lk-module-thread-add-message-button col-12"
                rel-method="addMessage">Отправить</span>
        </div>
        <div class="edit-control d-none">
            <span class="btn lk-module-thread-cancel-edit-message-button col-12 col-lg-12">Отменить</span>
            <span class="btn btn-success lk-module-thread-update-message-button col-12 col-lg-12"
                rel-method="updateMessage">Обновить</span>
        </div>
    </div>
    <div class="col-12 col-lg-6">
        ...
        <table class="table data thread-list mb-4" cellpadding="1" cellspacing="1">
            <tbody>
                ...
                <tr rel-message-id="<?= $message['id']; ?>" rel-sender-id="<?= $message['sender']; ?>"
                    rel-message="<?= $message['message']; ?>"
                    rel-reply-to-message-id="<?= (!is_null($message['reply_to_message_id']) ? $message['reply_to_message_id'] : ''); ?>"
                    class="thread-line<?= (($message['type'] === WebUserThreadMessagesModel::ITEM_TYPE_MESSAGE) ? ' message' . ((intval($message['sender']) === 0) ? ' outcoming' : ' incoming') : ' status'); ?><?= (intval($message['deleted']) === WebUserThreadMessagesModel::ITEM_STATUS_DELETED ? ' remove' : '') ?>">
                    <td>
                        <span>
                            <?php if ($message['reply_to']) : ?>
                            <span
                                class="reply-block d-block border-left border-secondary pb-1 px-2 my-2 mx-2 font-italic"><?= $message['reply_to']['message']; ?></span>
                            <?php endif; ?>
                            ...
                            <ul class="files">
                                ...
                                <li class="available" rel-file-id="<?= $file['id']; ?>"
                                    rel-file-path="/<?= $file['real_file_name']; ?>"
                                    rel-file-size="<?= $file['size']; ?>" rel-file-name="<?= $file['file_name']; ?>"
                                    rel-file-type="<?= $file['mime']; ?>">
                                    <a href="/<?= $file['real_file_path']; ?>" target="_blank"
                                        title="<?= $file['file_name']; ?>"><i class="fa fa-cloud-download-alt"></i>
                                        <?= $file['file_name']; ?></a>
                                </li>
                                <?php else : ?>
                            ...
                            <span class="controls">
                                ...
                                <a class="delete lk-module-thread-delete-message-button"
                                    rel-method="deleteMessage">Удалить</a>
                                <?php endif; ?>
                                <span class="date">
...
<script src="<?= App::getPublicWebRoot('/assets/libs/dropzone-5.9.3/ru.dropzone.js'); ?>?v=<?= time(); ?>"
    type="text/javascript"></script>
<script src="<?= App::getPublicWebRoot('/widgets/views/threadLine/assets/scripts/script.js'); ?>?v=<?= time(); ?>"
    type="text/javascript"></script>
