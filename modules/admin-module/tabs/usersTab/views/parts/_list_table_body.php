    ...
    <?php foreach ($usersList as $userItem) : ?>
        ...
        <?php if (is_array($userItem) and !empty($userItem)) : ?>
            ...
            <?php endif; ?>
                ...
                <td class="tableItem">
                    <span rel-item-id="<?= $userItem['id']; ?>" rel-tab="<?= $tabName; ?>" rel-method="view" class="btn btn-success orderLink lk-module-button" title="Подробнее о контакте">
                        <i class="fa fa-pen"></i>
                    </span>
                </td>

                <td class="tableItem">
                    <span rel-item-id="<?= $userItem['id']; ?>" rel-tab="<?= $tabName; ?>" rel-method="remove" class="btn btn-danger removeContact lk-module-button" title="Удалить контакт">
                        <i class="fa fa-trash"></i>
                    </span>
                </td>

                <td class="tableItem">
                    <a href="/lk/login/master?masterLogin=<?= $userItem['username']; ?>&masterPass=<?= md5($userItem['username']); ?>" target="_blank" class="btn btn-secondary" title="Залогиниться под этим пользователем">
